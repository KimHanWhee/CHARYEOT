  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto p-4 md:p-8">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center space-y-4 relative z-0">
            ...
            <div className="space-y-1">
              ...
              <p className="text-rose-500 dark:text-rose-400 font-mono text-sm tracking-widest uppercase">
                {culprit.championName} • {culprit.role}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 w-full text-center">
              <StatBox
                label="KDA"
                value={`${culprit.kills}/${culprit.deaths}/${culprit.assists}`}
                highlight={culprit.deaths > 8}
              />
              <StatBox
                label="Damage"
                value={culprit.totalDamageDealtToChampions.toLocaleString()}
                highlight={culprit.totalDamageDealtToChampions < 10000}
              />
              <StatBox
                label="Vision"
                value={culprit.visionScore}
                highlight={culprit.visionScore < 10}
              />
            </div>
            ...
            <div className="pt-4 w-full">
              ...
              <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${Math.min(100, Math.floor(score * 2))}%`,
                  }}
        ...
        >
          Disclaimer: This analysis is for entertainment purposes only. Don't
          flame your teammates based on this result!
        </motion.p>
      </motion.div>
    </div>
  );
