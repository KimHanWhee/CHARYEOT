...
export function MatchCard({
  ...
  const kda =
    searchUser.deaths === 0
      ? "Perfect"
      : ((searchUser.kills + searchUser.assists) / searchUser.deaths).toFixed(
          2,
        );
  ...
  return (
    <div
      className={cn(
        ...
        searchUser.win ? "border-emerald-500" : "border-rose-500",
      )}
    >
      {/* 요약 행 */}
      <motion.div
      ...
      >
        ...
        <div className="flex-shrink-0 whitespace-nowrap min-w-36">
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            {getTimeAgo(match.gameEndTimestamp)}
          </p>
          <h3
            className={cn(
              "text-lg font-bold",
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            {match.gameMode} • {Math.floor(match.duration / 60)}분{" "}
            {Math.floor(match.duration % 60)}초
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative w-16 h-16 rounded-lg overflow-hidden shadow-inner flex-shrink-0">
            ...
            />
            <span className="absolute bottom-0 right-0 bg-slate-800/50 dark:bg-slate-600/50 text-white text-xs font-bold px-1.5 py-0.5 rounded-tl border-t border-l border-slate-700 dark:border-slate-500">
              {searchUser.champLevel}
            </span>
          </div>
          ...
          <div className="hidden sm:flex gap-1">
            {Array.from({ length: 7 }).map((_, i) => {
              const itemId = searchUser[
                `item${i}` as keyof typeof searchUser
              ] as number;
              return (
                <div
                  key={i}
                  className="w-7 h-7 rounded overflow-hidden bg-slate-200 dark:bg-slate-700"
                >
                  {itemId !== 0 ? (
                    <img
        ...
        <div className="flex items-center gap-8 text-slate-700 dark:text-slate-200">
          <div className="text-center flex-shrink-0">
            <p className="text-lg font-mono font-bold flex items-center justify-center gap-1">
              ...
              <span>/</span>
              <span className="inline-block w-7 text-center text-rose-500 dark:text-rose-400">
                {searchUser.deaths}
              </span>
              <span>/</span>
          ...
          <div className="hidden md:flex gap-4 text-xs text-slate-500 dark:text-slate-400">
            <div className="flex flex-col items-center gap-1 flex-shrink-0 min-w-14">
              ...
              <span className="whitespace-nowrap">
                {searchUser.totalDamageDealtToChampions.toLocaleString()}
              </span>
            </div>
            ...
            <div className="flex flex-col items-center gap-1 flex-shrink-0 min-w-12">
              <Coins size={14} />
              <span className="whitespace-nowrap">
                {searchUser.totalMinionsKilled} CS
              </span>
            </div>
          </div>
