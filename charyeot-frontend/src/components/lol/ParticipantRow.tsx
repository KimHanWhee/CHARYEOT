...
// champion(2rem) | spells(1rem) | runes(1rem) | name(1fr) | KDA(90px) | dealt(3.5rem) | taken(3.5rem) | cs(2.5rem) | items(180px = 7×24px + 6×2px)
const GRID_COLS = '2rem 1rem 1rem minmax(5rem, 1fr) 90px 3.5rem 3.5rem 2.5rem 180px';
...
export function TeamHeader({ isWin, team }: { isWin: boolean; team: 'blue' | 'red' }) {
  return (
    <div
    ...
    >
      ...
      <span />
      <span className="text-center text-[10px] text-slate-500 dark:text-slate-400">K / D / A</span>
      <span className="text-center text-[10px] text-slate-500 dark:text-slate-400">가한 피해</span>
      <span className="text-center text-[10px] text-slate-500 dark:text-slate-400">받은 피해</span>
      <span className="text-center text-[10px] text-slate-500 dark:text-slate-400">CS</span>
...
export function ParticipantRow({ key, participant, version, isTarget }: ParticipantRowProps) {
  ...
  const handleNameClick = (e: { stopPropagation: () => void }) => {
    e.stopPropagation();
    navigate(`/${GAME_PATHS.LOL}/search/${encodeURIComponent(participant.riotIdGameName)}/${encodeURIComponent(participant.riotIdTagline)}`);
  };
